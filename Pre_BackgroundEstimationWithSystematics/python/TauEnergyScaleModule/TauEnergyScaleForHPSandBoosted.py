...
class TauEnergyScaleForHPSandBoosted(Module):
	def __init__(self, year, isData=False, tauID_wp='Loose', ele_wp='VVLoose'):
		...
		self.tesTool = TauESTool('UL'+self.year, 'DeepTau2018v2p5VSjet', wp=tauID_wp, wp_vsele=ele_wp)
	...
	def analyze(self, event):
		...
		for tau in taus:
			...
			#if ((tau.idDeepTau2018v2p5VSjet >= 4 and tau.idDeepTau2018v2p5VSe >= 2) and (self.isMC)):
			if ((self.isMC)):
				...
				# If the tau doesn't pass the WPs, keep the original pt and mass
				pt_nom.append(pt)
