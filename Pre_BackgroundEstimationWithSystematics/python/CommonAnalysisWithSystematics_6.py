...
class cutsAndcategories(Module):
	def __init__(self,filename,year,isData,runNominal=False):
		...
		if ((self.isData) or (self.runNominal)):
			...
			self.jesUnc = ["","jesTotalUp","jesTotalDown","jerUp","jerDown",
						   ...
						   "UnclustUp","UnclustDown","tesUp","tesDown"]
	...
	def analyze(self, event):
		...
		def getjetpt(jet, sys):
			if ((sys == "") or (sys == "UnclustUp") or (sys == "UnclustDown") or (sys == "tesUp") or (sys == "tesDown")):
				return jet.pt_nom
			elif sys == "jesTotalUp":
		...
		for sys in self.jesUnc:
			...
			Tau_enu = filter(lambda x: (gettaupt(x[1],sys) > 20) and (abs(x[1].eta) < 2.5) and (abs(x[1].dz)<0.2) and (x[1].idDecayModeNewDMs) and (x[1].idDeepTau2018v2p5VSjet >= 4) and (x[1].idDeepTau2018v2p5VSe >= 2) and (x[1].idDeepTau2018v2p5VSmu >= 1), enumerate(Tau)) #The HPS tau id are not bitmps anymore
			
			
			#boostedTau_enu = filter(lambda x: (x[1].pt > 20) and (abs(x[1].eta) < 2.5) and (x[1].rawDeepTau2018v2p7VSjet>=0.85), enumerate(boostedTau))
			boostedTau_enu = filter(lambda x: (gettaupt(x[1],sys) > 20) and (abs(x[1].eta) < 2.5) and (x[1].rawDeepTau2018v2p7VSjet>=0.85), enumerate(boostedTau))
			...
			#if self.muID == 2:
			Muon_enu = filter(lambda x: x[1].pt > 15 and (abs(x[1].eta) < 2.4) and x[1].looseId, enumerate(Muon))
			#elif self.muID == 3:
			...
			if Keymax == "bb":
				...
				secondLepton =  fastMTTlepton(pt = gettaupt(Tau[gTau_index[1]],sys),eta = Tau[gTau_index[1]].eta,phi = Tau[gTau_index[1]].phi,m = gettaumass(Tau[gTau_index[1]],sys),leptonType = 'Tau',tauDecayMode=Tau[gTau_index[1]].decayMode)
				self.pair2FV.SetPtEtaPhiM(gettaupt(Tau[gTau_index[1]],sys), Tau[gTau_index[1]].eta,Tau[gTau_index[1]].phi,gettaumass(Tau[gTau_index[0]],sys))
...
def call_postpoc(files):
	...
	if (search("Run", filename)):
		...
		tesModule = lambda: TauEnergyScaleForHPSandBoosted(args.year,True,tauID_wp='Loose', ele_wp='VVLoose')
		...
		tesModule = lambda: TauEnergyScaleForHPSandBoosted(args.year,False,tauID_wp='Loose', ele_wp='VVLoose')
	...
	if (args.Remove):
		if (search("Run", filename)):
			p = PostProcessor(args.tempStore,[files], cut=preselection, branchsel='Keep_Drop_txt/Data_remove_oldWtnaming.txt',modules=[tesModule(),mainModule()], postfix=post,noOut=False,outputbranchsel='Keep_Drop_txt/Data_remove_oldWtnaming.txt')#,histFileName="{}/cutflow2_{}.root".format(args.tempStore, filename),histDirName="cutflow2")
			...
			p = PostProcessor(args.tempStore,[files], cut=preselection, branchsel='Keep_Drop_txt/MC_remove_oldWtnaming.txt',modules=[tesModule(),mainModule()], postfix=post,noOut=False,outputbranchsel='Keep_Drop_txt/MC_remove_oldWtnaming.txt')#,maxEntries=12000)#,histFileName="{}/cutflow2_{}.root".format(args.tempStore, filename),histDirName="cutflow2")
