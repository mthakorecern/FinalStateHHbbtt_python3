...
class cutsAndcategories(Module):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        for sys in self.jesUnc:
            ...
            self.out.branch(
                ...
                (sys),
                "I",
                lenVar="ngood_Electrons%s" %
                (sys))
            self.out.branch(
                "index_gMuons%s" %
                (sys),
                "I",
                lenVar="ngood_Muons%s" %
                (sys))
            self.out.branch(
                "index_gTaus%s" %
                (sys),
                ...
                (sys))
            self.out.branch(
                "index_gboostedTaus%s" %
                (sys),
                "I",
                lenVar="ngood_boostedTaus%s" %
                (sys))
            self.out.branch(
                "index_gFatJets%s" %
                (sys),
                "I",
                lenVar="ngood_FatJets%s" %
                (sys))
            self.out.branch(
                "index_gJets%s" %
                (sys),
            ...
            self.out.branch(
                ...
                (sys),
                "I",
                lenVar="ngood_MediumJets%s" %
                (sys))
            self.out.branch(
                "index_gTightJets%s" %
                (sys),
                "I",
                lenVar="ngood_TightJets%s" %
                (sys))
    ...
    def analyze(self, event):
        ...
        for sys in self.jesUnc:
            ...
            Jet_enu = [x for x in enumerate(
                Jet) if applyPOGselectionToAK4(x, sys)]
            ...
            Tau_enu = [
                x for x in Tau_enu if ElectronTauOverlap(
            ...
            Tau_enu = [
                x for x in Tau_enu if MuonTauOverlap(
                    x, Muon_enu, boost=0)]
            boostedTau_enu = [
            ...
            if sys == "":
                ...
                self.out.fillBranch(
                    "HTTvis_deltaR%s" %
                    (sys), self.pair1FV.DeltaR(
                        self.pair2FV))

            if sys == "":
                self.out.fillBranch(
                    "Hbb_lep1_deltaR%s" %
                    (sys), self.higgsBBFV.DeltaR(
                        self.pair1FV))
                self.out.fillBranch(
                    "Hbb_lep2_deltaR%s" %
                    (sys), self.higgsBBFV.DeltaR(
...
def call_postpoc(files):
    ...
    if (search("Run", filename)):
        ...
        def tesModule(): return TauCorrectionProducer(
