...
class cutsAndcategories(Module):
	def __init__(self,filename,year,isData,runNominal=False):
		...
		#Define the TLorentz vectors as memeber variables to avoid multiple reinitializations within loops
	...
	def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
		...
		if ((self.year == "2018") and (self.isMC==True)):
	...
	def analyze(self, event):
		...
		def getMETphi(sys):
 		...
 		def applyPOGselectionToAK4(ak4Object_enu,sys):
			...
			#if ((ak4Object_enu[1].pt_nom>30) and (abs(ak4Object_enu[1].eta)<2.5)):
			if ((getjetpt(ak4Object_enu[1],sys)>30) and (abs(ak4Object_enu[1].eta)<2.5)):
 		...
 		def applyPOGselectionToAK4_for2018Veto(ak4Object_enu):
			...
			#Link: https://cms-pub-talk.web.cern.ch/t/jme-or/29619/4
			if ((ak4Object_enu[1].pt_nom>15) and (ak4Object_enu[1].eta < -1.3) and (ak4Object_enu[1].eta > -3.2) and (ak4Object_enu[1].phi < -0.87) and (ak4Object_enu[1].phi > -1.57)):
				if (ak4Object_enu[1].jetId>1):
					return True
			return False
		...
		def pass_cuts_EleID(electronObject_enu):
			for  cutnr in range(0,10):
				if cutnr==7:
					continue
				#if (electronObject_enu[1].vidNestedWPBitmap >> (cutnr*3) & 0x7) < self.eleID:
				if (electronObject_enu[1].vidNestedWPBitmap >> (cutnr*3) & 0x7) < 2:
					return False
			return True

		def JetFatJetOverlap(jetObject_enu,sys): #FatJet and Jet overlap, separation > 1.2 (0.8 + 0.4)
		...
		def FatJetConeIsolation(leptonObject_enu): # Function used by electron(muon) cleaning vs AK8 > 0.8
			self.lepFV.SetPtEtaPhiM(leptonObject_enu[1].pt,leptonObject_enu[1].eta,leptonObject_enu[1].phi,leptonObject_enu[1].mass)
		...
		def FatJetTauOverlap(tauObject_enu,boost): # Function used by Tau(boostedTau) cleaning vs AK8 > 0.8
			if boost==1:
		...
		def ElectronTauOverlap(tauObject_enu,elecoll_enu,boost): # Function used by Tau(boostedTau) cleaning vs Electrons > 0.05
			if boost==1:
				self.tauFV.SetPtEtaPhiM(tauObject_enu[1].pt,tauObject_enu[1].eta,tauObject_enu[1].phi,tauObject_enu[1].mass)
			else:
				#self.tauFV.SetPtEtaPhiM(tauObject_enu[1].pt_nom,tauObject_enu[1].eta,tauObject_enu[1].phi,tauObject_enu[1].mass_nom)
		...
		def MuonTauOverlap(tauObject_enu,mucoll_enu,boost): # Function used by Tau(boostedTau) cleaning vs Muons > 0.05
			if boost==1:
				self.tauFV.SetPtEtaPhiM(tauObject_enu[1].pt,tauObject_enu[1].eta,tauObject_enu[1].phi,tauObject_enu[1].mass)
			else:
				#self.tauFV.SetPtEtaPhiM(tauObject_enu[1].pt_nom,tauObject_enu[1].eta,tauObject_enu[1].phi,tauObject_enu[1].mass_nom)
		...
		def selfPairing(col1,tag):
			...
			for i in range(len(col1)):
				for j in range(i+1,len(col1)):
					self.pair1FV.SetPtEtaPhiM(col1[i][1].pt,col1[i][1].eta,col1[i][1].phi,col1[i][1].mass)
					self.pair2FV.SetPtEtaPhiM(col1[j][1].pt,col1[j][1].eta,col1[j][1].phi,col1[j][1].mass)
		...
		def crossPairing(col1,col2,tag):
			...
			tagprotection = ((tag=="be") or (tag=="te") or (tag=="bm") or (tag=="tm"))
			...
			for i in range(len(col1)):
				for j in range(len(col2)):
					self.pair1FV.SetPtEtaPhiM(col1[i][1].pt,col1[i][1].eta,col1[i][1].phi,col1[i][1].mass)
					self.pair2FV.SetPtEtaPhiM(col2[j][1].pt,col2[j][1].eta,col2[j][1].phi,col2[j][1].mass)					
					deltaR_Pair = (self.pair1FV.DeltaR(self.pair2FV))
					if ((deltaR_Pair<=0.05)):
						continue
					#Found the bug in the code - I was only applying isolation for boostedTaus but none for HPS
					if ((tag == "be") or (tag == "te")):
						if not ElectronIsolationCut(col1[i][1],col2[j][1],tag):
							continue
		...
		def ElectronIsolationCut(tau,ele,tag): #passing the inidivial eletrons from the collection to apply the correction
			...
			if deltaR < 0.6:
				isTau = "close"
			if isTau == "close":
				self.leadingMatch.SetPtEtaPhiM(tau.LeadingElectronPt,tau.LeadingElectronEta,tau.LeadingElectronPhi,0.0)
				self.subleadingMatch.SetPtEtaPhiM(tau.SubLeadingElectronPt,tau.SubLeadingElectronEta,tau.SubLeadingElectronPhi,0.0)
				self.subsubleadingMatch.SetPtEtaPhiM(tau.SubSubLeadingElectronPt,tau.SubSubLeadingElectronEta,tau.SubSubLeadingElectronPhi,0.0)			
		...
		if (self.year == "2018"):			
			if ((self.isData==True) and (event.run>=319077)):
				...
				#Adding buffer region (in eta-phi) for AK8 Jet veto for HEM: https://mattermost.web.cern.ch/cms-hh-bbtautau/pl/6fk8on61rj8d7jnrwxnwj9uiph
				FatJet_Vetoenu = filter(lambda x:(x[1].eta < -1.1) and (x[1].eta > -2.7) and (x[1].phi < -0.67) and (x[1].phi > -1.77) and (x[1].jetId>1), enumerate(FatJet))				
				Electron_Vetoenu = filter(lambda x: x[1].pt > 10 and ((int(x[1].cutBased))>=2) and (x[1].eta < -1.3) and (x[1].eta > -2.5) and (x[1].phi < -0.87) and (x[1].phi > -1.57) , enumerate(Electron))
				if ((len(FatJet_Vetoenu)>0) or (len(Jet_Vetoenu)>0) or (len(Electron_Vetoenu)>0)):
					return False
			if ((self.isMC==True)):
		...
		if (self.isMC):
			if (self.year_unc=="2018"):
				min_upvar = min([event.METcorrected_ptScaleUp, event.METcorrected_ptScaleAbsoluteUp, event.METcorrected_ptScaleAbsolute_2018Up, event.METcorrected_ptScaleBBEC1Up, event.METcorrected_ptScaleBBEC1_2018Up, event.METcorrected_ptScaleEC2Up, event.METcorrected_ptScaleEC2_2018Up, event.METcorrected_ptScaleFlavorQCDUp, event.METcorrected_ptScaleHFUp, event.METcorrected_ptScaleHF_2018Up, event.METcorrected_ptScaleRelativeBalUp, event.METcorrected_ptScaleRelativeSample_2018Up, event.METcorrected_ptResUp, event.METcorrected_ptUnclustUp])
				min_downvar = min([event.METcorrected_ptScaleDown, event.METcorrected_ptScaleAbsoluteDown, event.METcorrected_ptScaleAbsolute_2018Down, event.METcorrected_ptScaleBBEC1Down, event.METcorrected_ptScaleBBEC1_2018Down, event.METcorrected_ptScaleEC2Down, event.METcorrected_ptScaleEC2_2018Down, event.METcorrected_ptScaleFlavorQCDDown, event.METcorrected_ptScaleHFDown, event.METcorrected_ptScaleHF_2018Down, event.METcorrected_ptScaleRelativeBalDown, event.METcorrected_ptScaleRelativeSample_2018Down, event.METcorrected_ptResDown, event.METcorrected_ptUnclustDown])
			elif (self.year_unc=="2017"):
				min_upvar = min([event.METcorrected_ptScaleUp, event.METcorrected_ptScaleAbsoluteUp, event.METcorrected_ptScaleAbsolute_2017Up, event.METcorrected_ptScaleBBEC1Up, event.METcorrected_ptScaleBBEC1_2017Up, event.METcorrected_ptScaleEC2Up, event.METcorrected_ptScaleEC2_2017Up, event.METcorrected_ptScaleFlavorQCDUp, event.METcorrected_ptScaleHFUp, event.METcorrected_ptScaleHF_2017Up, event.METcorrected_ptScaleRelativeBalUp, event.METcorrected_ptScaleRelativeSample_2017Up, event.METcorrected_ptResUp, event.METcorrected_ptUnclustUp])
				min_downvar = min([event.METcorrected_ptScaleDown, event.METcorrected_ptScaleAbsoluteDown, event.METcorrected_ptScaleAbsolute_2017Down, event.METcorrected_ptScaleBBEC1Down, event.METcorrected_ptScaleBBEC1_2017Down, event.METcorrected_ptScaleEC2Down, event.METcorrected_ptScaleEC2_2017Down, event.METcorrected_ptScaleFlavorQCDDown, event.METcorrected_ptScaleHFDown, event.METcorrected_ptScaleHF_2017Down, event.METcorrected_ptScaleRelativeBalDown, event.METcorrected_ptScaleRelativeSample_2017Down, event.METcorrected_ptResDown, event.METcorrected_ptUnclustDown])			
			elif (self.year_unc=="2016"):
				min_upvar = min([event.METcorrected_ptScaleUp, event.METcorrected_ptScaleAbsoluteUp, event.METcorrected_ptScaleAbsolute_2016Up, event.METcorrected_ptScaleBBEC1Up, event.METcorrected_ptScaleBBEC1_2016Up, event.METcorrected_ptScaleEC2Up, event.METcorrected_ptScaleEC2_2016Up, event.METcorrected_ptScaleFlavorQCDUp, event.METcorrected_ptScaleHFUp, event.METcorrected_ptScaleHF_2016Up, event.METcorrected_ptScaleRelativeBalUp, event.METcorrected_ptScaleRelativeSample_2016Up, event.METcorrected_ptResUp, event.METcorrected_ptUnclustUp])
				min_downvar = min([event.METcorrected_ptScaleDown, event.METcorrected_ptScaleAbsoluteDown, event.METcorrected_ptScaleAbsolute_2016Down, event.METcorrected_ptScaleBBEC1Down, event.METcorrected_ptScaleBBEC1_2016Down, event.METcorrected_ptScaleEC2Down, event.METcorrected_ptScaleEC2_2016Down, event.METcorrected_ptScaleFlavorQCDDown, event.METcorrected_ptScaleHFDown, event.METcorrected_ptScaleHF_2016Down, event.METcorrected_ptScaleRelativeBalDown, event.METcorrected_ptScaleRelativeSample_2016Down, event.METcorrected_ptResDown, event.METcorrected_ptUnclustDown])

			#Now reject events (skim further)
			if ((event.METcorrected_pt < 180) and (min_upvar < 180) and (min_downvar < 180)):
				return False
		...
		FatJet_skim_enu = filter(lambda x: (x[1].pt_nom >= 180) and (abs(x[1].eta) < 2.5) and (x[1].jetId>1), enumerate(FatJet))
		...
		for sys in self.jesUnc:
			...
			FatJet_enu = filter(lambda x: (getjetpt(x[1],sys) >= 200) and (abs(x[1].eta) < 2.5) and (x[1].jetId>1), enumerate(FatJet))
			if (len(FatJet_enu)==0):
			...
			zipPair = zip(FatJet_enu,HbbPtList)
			FatJet_enu = [fatjetobject_enu for fatjetobject_enu,_ in sorted(zipPair,key= lambda x:x[1],reverse=True)]

			#remove the HbbPtList, zipPair
			del HbbPtList
			del zipPair
	
			Jet_enu = filter(lambda x:applyPOGselectionToAK4(x,sys),enumerate(Jet))
			...
			#Without TES applied
			Tau_enu = filter(lambda x: (x[1].pt > 20) and (abs(x[1].eta) < 2.5) and (abs(x[1].dz)<0.2) and (x[1].idDecayModeNewDMs) and (x[1].idDeepTau2018v2p5VSjet >= 4) and (x[1].idDeepTau2018v2p5VSe >= 2) and (x[1].idDeepTau2018v2p5VSmu >= 1), enumerate(Tau)) #The HPS tau id are not bitmps anymore
			...
			Electron_enu = filter(lambda x: (x[1].pt > 10) and ((abs(x[1].eta) <= 1.44) or (abs(x[1].eta) >= 1.57)), enumerate(Electron))
			Electron_enu = filter(pass_cuts_EleID, Electron_enu)
			...
			#if self.muID == 2:
			Muon_enu = filter(lambda x: x[1].pt > 10 and (abs(x[1].eta) < 2.4) and x[1].looseId, enumerate(Muon))
			#elif self.muID == 3:
			...
			Jet_enu = filter(lambda x:JetFatJetOverlap(x,sys),Jet_enu)

			Electron_enu = filter(FatJetConeIsolation,Electron_enu)
			Muon_enu = filter(FatJetConeIsolation,Muon_enu)

			Tau_enu = filter(lambda x:FatJetTauOverlap(x,boost=0),Tau_enu)
			boostedTau_enu = filter(lambda x:FatJetTauOverlap(x,boost=1), boostedTau_enu)

			Tau_enu = filter(lambda x:ElectronTauOverlap(x,Electron_enu,boost=0),Tau_enu)
			boostedTau_enu = filter(lambda x:ElectronTauOverlap(x,Electron_enu,boost=1),boostedTau_enu)

			Tau_enu = filter(lambda x:MuonTauOverlap(x,Muon_enu,boost=0),Tau_enu)
			boostedTau_enu = filter(lambda x:MuonTauOverlap(x,Muon_enu,boost=1),boostedTau_enu)
			...
			if sys == "":
				...
				if (len(Electron_enu)>0):
					self.cutflow_dict[" ...breakdown..> Atleast_one_Electron (Reco + IDnoIso + cleaning)"] += 1	
			...
			if (list[Keymax][0]<0):
				#return False
				#Electron and Muon isolation failed - move on to the next systematics
				fillBranchesWithDefault(sys)
				continue
			...
			if Keymax == "bb":
				...
				self.pair1FV.SetPtEtaPhiM(boostedTau[gboostedTau_index[0]].pt, boostedTau[gboostedTau_index[0]].eta,boostedTau[gboostedTau_index[0]].phi,boostedTau[gboostedTau_index[0]].mass)
				secondLepton =  fastMTTlepton(pt = boostedTau[gboostedTau_index[1]].pt,eta = boostedTau[gboostedTau_index[1]].eta,phi = boostedTau[gboostedTau_index[1]].phi,m = boostedTau[gboostedTau_index[1]].mass,leptonType = 'Tau',tauDecayMode=boostedTau[gboostedTau_index[1]].decayMode)
				self.pair2FV.SetPtEtaPhiM(boostedTau[gboostedTau_index[1]].pt, boostedTau[gboostedTau_index[1]].eta,boostedTau[gboostedTau_index[1]].phi,boostedTau[gboostedTau_index[1]].mass)
				...
				if (sys==""):
					self.out.fillBranch("allTaus_decayMode%s"%(sys),[Tau[gTau_index[0]].decayMode,Tau[gTau_index[0]].decayMode])			
				...
				self.pair1FV.SetPtEtaPhiM(Tau[gTau_index[0]].pt, Tau[gTau_index[0]].eta,Tau[gTau_index[0]].phi,Tau[gTau_index[0]].mass)
				secondLepton =  fastMTTlepton(pt = Tau[gTau_index[1]].pt,eta = Tau[gTau_index[1]].eta,phi = Tau[gTau_index[1]].phi,m = Tau[gTau_index[1]].mass,leptonType = 'Tau',tauDecayMode=Tau[gTau_index[1]].decayMode)
				self.pair2FV.SetPtEtaPhiM(Tau[gTau_index[1]].pt, Tau[gTau_index[1]].eta,Tau[gTau_index[1]].phi,Tau[gTau_index[1]].mass)
			...
			pass_quality_delcuts = (((abs(self.higgsBBFV.DeltaPhi(self.met)))>1) and ((self.pair1FV.DeltaR(self.pair2FV))>0) and  ((self.pair1FV.DeltaR(self.pair2FV))<1.5))
			...
			self.out.fillBranch("Hbb_met_phi%s"%(sys),self.higgsBBFV.DeltaPhi(self.met))
			...
			Jet_enu = filter(lambda x:removeOverlapOfAK4WithLightHeavyLeptons(x,gTau_index,Tau,gboostedTau_index,boostedTau,gElectron_index,Electron,gMuon_index,Muon,sys),Jet_enu)
			...
			if sys == "":
				...
				self.out.fillBranch("HTTvis_m%s"%(sys),self.higgsTTvisFV.M())
				self.out.fillBranch("HTTvis_eta%s"%(sys),self.higgsTTvisFV.Eta())
				self.out.fillBranch("HTTvis_phi%s"%(sys),self.higgsTTvisFV.Phi())
				self.out.fillBranch("HTTvis_pt%s"%(sys),self.higgsTTvisFV.Pt())
				self.out.fillBranch("HTTvis_deltaR%s"%(sys),self.pair1FV.DeltaR(self.pair2FV))

			if sys == "":
				self.out.fillBranch("Hbb_lep1_deltaR%s"%(sys),self.higgsBBFV.DeltaR(self.pair1FV))
				self.out.fillBranch("Hbb_lep2_deltaR%s"%(sys),self.higgsBBFV.DeltaR(self.pair2FV))
				self.out.fillBranch("softdropmassnom%s"%(sys),FatJet_enu[0][1].msoftdrop_nom)
				self.out.fillBranch("softdropmass%s"%(sys),FatJet_enu[0][1].msoftdrop)
				self.out.fillBranch("pnetmass%s"%(sys),FatJet_enu[0][1].particleNetLegacy_mass)
			...
			if sys == "":     
				...
				self.out.fillBranch("Xvis_eta%s"%(sys),self.RadionvisFV.Eta())
				self.out.fillBranch("Xvis_phi%s"%(sys),self.RadionvisFV.Phi())
			...
			self.out.fillBranch("ngood_boostedTaus%s"%(sys),len(gboostedTau_index))
			self.out.fillBranch("ngood_Taus%s"%(sys),len(gTau_index))
			self.out.fillBranch("ngood_Electrons%s"%(sys),len(gElectron_index))
			...
			self.out.fillBranch("ngood_MediumJets%s"%(sys),len(gJet_Mediumindex))
			self.out.fillBranch("ngood_TightJets%s"%(sys),len(gJet_Tightindex))
			...
			self.out.fillBranch("index_gboostedTaus%s"%(sys),gboostedTau_index)
			...
			self.out.fillBranch("index_gMediumJets%s"%(sys),gJet_Mediumindex)
			self.out.fillBranch("index_gTightJets%s"%(sys),gJet_Tightindex)

			if ((Keymax == "bb") or (Keymax == "tt")):
				if (sys==""):
					self.out.fillBranch("allTaus_pt%s"%(sys),[self.pair1FV.Pt(),self.pair2FV.Pt()])
					self.out.fillBranch("allTaus_eta%s"%(sys),[self.pair1FV.Eta(),self.pair2FV.Eta()])
					self.out.fillBranch("allTaus_phi%s"%(sys),[self.pair1FV.Phi(),self.pair2FV.Phi()])
					self.out.fillBranch("allTaus_mass%s"%(sys),[self.pair1FV.M(),self.pair2FV.M()])
				...
				if (sys==""):
					self.out.fillBranch("allTaus_pt%s"%(sys),[self.pair1FV.Pt()])
					self.out.fillBranch("allTaus_eta%s"%(sys),[self.pair1FV.Eta()])
					self.out.fillBranch("allTaus_phi%s"%(sys),[self.pair1FV.Phi()])
					self.out.fillBranch("allTaus_mass%s"%(sys),[self.pair1FV.M()])
				#if ((len(Electron_addlep_enu)!=0) or (len(Muon_addlep_enu)!=0)):
				#	self.out.fillBranch("addlepton_vetoflag_all",1)
...
def call_postpoc(files):
	...
	if (search("Run", filename)):
		...
		tesModule = lambda: TauCorrectionProducer(args.year,True,tauID_wp='Loose', ele_wp='VVLoose')
		#mainModule = lambda: cutsAndcategories(filename,args.year,True,args.eleID,args.muID,args.tauID,args.btauID)
		mainModule = lambda: cutsAndcategories(filename,args.year,True,args.runNominal)
		...
		#tesModule = lambda: TauCorrectionProducer(args.year,False,tauID_wp='Loose', ele_wp='VVLoose')
		#mainModule = lambda: cutsAndcategories(filename,args.year,False,args.eleID,args.muID,args.tauID,args.btauID)
		mainModule = lambda: cutsAndcategories(filename,args.year,False,args.runNominal)
	...
	if (args.Remove):
		if (search("Run", filename)):
			p = PostProcessor(args.tempStore,[files], cut=preselection, branchsel='Keep_Drop_txt/Data_remove_oldWtnaming.txt',modules=[mainModule()], postfix=post,noOut=False,outputbranchsel='Keep_Drop_txt/Data_remove_oldWtnaming.txt')#,histFileName="{}/cutflow2_{}.root".format(args.tempStore, filename),histDirName="cutflow2")
			...
			p = PostProcessor(args.tempStore,[files], cut=preselection, branchsel='Keep_Drop_txt/MC_remove_oldWtnaming.txt',modules=[mainModule()], postfix=post,noOut=False,outputbranchsel='Keep_Drop_txt/MC_remove_oldWtnaming.txt')#,maxEntries=12000)#,histFileName="{}/cutflow2_{}.root".format(args.tempStore, filename),histDirName="cutflow2")
		...
		p = PostProcessor(args.tempStore,[files], cut=None, branchsel=None,modules=[mainModule(),fatjetVarModule(),wandzWtModule()], postfix=post,noOut=False,outputbranchsel=None)
	p.run()	
	...
	if (args.transferOff):
		...
		cutflowDir = os.path.join(outputDir, "cutflow")
		if not os.path.exists(cutflowDir):
			os.makedirs(cutflowDir)

		# Move the cutflow rootfile
		os.system("mv " + args.tempStore + "/" + "cutflow2_" + filename + ".json " + cutflowDir + "/.")
...
if __name__ == "__main__":
	...
	#parser.add_argument('--btauID','-btId',help="DeepBoostedTau Raw score",default='0.85')#default is 0.85
	...
	trigger_2017_18=["HLT_PFMETNoMu120_PFMHTNoMu120_IDTight","HLT_MonoCentralPFJet80_PFMETNoMu120_PFMHTNoMu120_IDTight"]
