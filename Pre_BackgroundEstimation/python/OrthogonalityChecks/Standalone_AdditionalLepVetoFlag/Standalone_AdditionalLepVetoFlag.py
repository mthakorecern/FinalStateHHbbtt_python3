...
class Standalone_AdditionalLepVetoFlag(Module):
	...
	def analyze(self, event):
		...
		self.higgsBBFV.SetPtEtaPhiM(FatJet[event.index_gFatJets[0]].pt_nom,FatJet[event.index_gFatJets[0]].eta,FatJet[event.index_gFatJets[0]].phi,FatJet[event.index_gFatJets[0]].mass_nom)
		...
		Electron_addlep_enu = filter(distinctFromExistinglepton,Electron_addlep_enu)
		...
		Muon_addlep_enu = filter(distinctFromExistinglepton,Muon_addlep_enu)
		...
		self.out.fillBranch("index_gVetoElectrons",[x[0] for x in Electron_addlep_enu])
		self.out.fillBranch("nvetoMuon",len(Muon_addlep_enu))
		self.out.fillBranch("index_gVetoMuons",[x[0] for x in Muon_addlep_enu])	
		return True
...
def call_postpoc(files):
	...
	if (search("Run", filename)):
		...
		mainModule = lambda: Standalone_AdditionalLepVetoFlag(args.year,True)
	else:
		print ("This is a ",args.year," MC file = ", filename)
		mainModule = lambda: Standalone_AdditionalLepVetoFlag(args.year,False)
...
if __name__ == "__main__":
	...
	fnames = glob.glob(args.inputLocation + "/Radion*.root")  #making a list of input MC/DATA files
