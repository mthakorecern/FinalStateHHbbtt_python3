...
class cutsAndcategories(Module):
	...
	def analyze(self, event):
		...
		def ElectronTauOverlap(tauObject_enu,elecoll_enu): # Function used by Tau(boostedTau) cleaning vs Electrons > 0.05
			self.tauFV.SetPtEtaPhiM(tauObject_enu[1].pt,tauObject_enu[1].eta,tauObject_enu[1].phi,tauObject_enu[1].mass)
		...
		def MuonTauOverlap(tauObject_enu,mucoll_enu): # Function used by Tau(boostedTau) cleaning vs Muons > 0.05
			self.tauFV.SetPtEtaPhiM(tauObject_enu[1].pt,tauObject_enu[1].eta,tauObject_enu[1].phi,tauObject_enu[1].mass)
		...
		def removeOverlapOfAK4WithLightHeavyLeptons(ak4Object_enu,gTau_index,Tau_coll,gbTau_index,bTau_coll,gEle_index,Ele_coll,gMu_index,Mu_coll):
		...
		Tau_enu = filter(lambda x: (x[1].pt > 20) and (abs(x[1].eta) < 2.5) and (abs(x[1].dz)<0.2) and (x[1].idDecayModeNewDMs) and (x[1].idDeepTau2018v2p5VSjet & self.tauID == self.tauID), enumerate(Tau))

		boostedTau_enu = filter(lambda x: (x[1].pt > 20) and (abs(x[1].eta) < 2.5) and (x[1].rawDeepTau2018v2p7VSjet>=self.btauID), enumerate(boostedTau))

		FatJet_enu = filter(lambda x: (x[1].pt_nom > 250) and (abs(x[1].eta) < 2.5) and (x[1].jetId>1), enumerate(FatJet))
		if (len(FatJet_enu)==0):
			return False
		...
		Electron_enu = filter(lambda x: x[1].pt > 10, enumerate(Electron))
		Electron_enu = filter(pass_cuts_EleID, Electron_enu)

		if self.muID == 2:
			Muon_enu = filter(lambda x: x[1].pt > 10 and (abs(x[1].eta) < 2.5) and x[1].looseId, enumerate(Muon))
		elif self.muID == 3:
		...
		Tau_enu = filter(lambda x:ElectronTauOverlap(x,Electron_enu),Tau_enu)
		boostedTau_enu = filter(lambda x:ElectronTauOverlap(x,Electron_enu),boostedTau_enu)

		Tau_enu = filter(lambda x:MuonTauOverlap(x,Muon_enu),Tau_enu)
		boostedTau_enu = filter(lambda x:MuonTauOverlap(x,Muon_enu),boostedTau_enu)
		...
		if Keymax == "bb":
			...
			self.out.fillBranch("allTaus_decayMode",[boostedTau[gboostedTau_index[0]].decayMode,boostedTau[gboostedTau_index[1]].decayMode])			
			...
			self.out.fillBranch("allTaus_decayMode",[Tau[gTau_index[0]].decayMode,Tau[gTau_index[0]].decayMode])			
			...
			self.out.fillBranch("allTaus_decayMode",[boostedTau[gboostedTau_index[0]].decayMode])
			firstLepton  =  fastMTTlepton(pt = boostedTau[gboostedTau_index[0]].pt,eta = boostedTau[gboostedTau_index[0]].eta,phi = boostedTau[gboostedTau_index[0]].phi,m = boostedTau[gboostedTau_index[0]].mass,leptonType = 'Tau',tauDecayMode=boostedTau[gboostedTau_index[0]].decayMode)
			self.pair1FV.SetPtEtaPhiM(boostedTau[gboostedTau_index[0]].pt, boostedTau[gboostedTau_index[0]].eta,boostedTau[gboostedTau_index[0]].phi,boostedTau[gboostedTau_index[0]].mass)			
			...
			gElectron_index = [list[Keymax][2]]	
			self.out.fillBranch("allTaus_decayMode",[Tau[gTau_index[0]].decayMode])
			firstLepton  =  fastMTTlepton(pt = Tau[gTau_index[0]].pt,eta = Tau[gTau_index[0]].eta,phi = Tau[gTau_index[0]].phi,m = Tau[gTau_index[0]].mass,leptonType = 'Tau',tauDecayMode=Tau[gTau_index[0]].decayMode)
			self.pair1FV.SetPtEtaPhiM(Tau[gTau_index[0]].pt, Tau[gTau_index[0]].eta,Tau[gTau_index[0]].phi,Tau[gTau_index[0]].mass)
			secondLepton =  fastMTTlepton(pt = Electron[gElectron_index[0]].pt,eta = Electron[gElectron_index[0]].eta,phi = Electron[gElectron_index[0]].phi,m = 0.51100e-3,leptonType = 'Electron',tauDecayMode=-1)
			self.pair2FV.SetPtEtaPhiM(Electron[gElectron_index[0]].pt, Electron[gElectron_index[0]].eta, Electron[gElectron_index[0]].phi,0.0)
		...
		Jet_enu = filter(lambda x:removeOverlapOfAK4WithLightHeavyLeptons(x,gTau_index,Tau,gboostedTau_index,boostedTau,gElectron_index,Electron,gMuon_index,Muon),Jet_enu)
		...
		if ((Keymax == "bb") or (Keymax == "tt")):
			self.out.fillBranch("allTaus_pt",[self.pair1FV.Pt(),self.pair2FV.Pt()])
			self.out.fillBranch("allTaus_eta",[self.pair1FV.Eta(),self.pair2FV.Eta()])
			self.out.fillBranch("allTaus_phi",[self.pair1FV.Phi(),self.pair2FV.Phi()])
			self.out.fillBranch("allTaus_mass",[self.pair1FV.M(),self.pair2FV.M()])
		else:
...
def call_postpoc(files):
	...
	if (search("Run", filename)):
		...
		mainModule = lambda: cutsAndcategories(args.year,True,args.eleID,args.muID,args.tauID,args.btauID)
	else:
		print ("This is a ",args.year," MC file = ", filename)
		mainModule = lambda: cutsAndcategories(args.year,False,args.eleID,args.muID,args.tauID,args.btauID)
	
	p = PostProcessor(args.tempStore,[files], cut=None, branchsel=None,modules=[mainModule()], postfix=post,noOut=False,outputbranchsel=None)
	p.run()	
...
if __name__ == "__main__":
	...
	parser.add_argument('--btauID','-btId',help="DeepBoostedTau Raw score",required=True)
