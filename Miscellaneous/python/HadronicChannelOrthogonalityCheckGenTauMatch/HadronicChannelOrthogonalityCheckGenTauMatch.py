...
for mass in mass_points:
    ...
    for signal in signals:
        ...
        if signal == "HHbtautau":
            file_path = "{}/RadionTohhTohtatahbb_narrow_M-{}.root".format(
                base_path, str(mass))
        elif signal == "HHbbWW1L1Nu2Q":
            file_path = "{}/RadionHHbbwww_Lnu2Q_narrow_M-{}.root".format(
                base_path, str(mass))
        elif signal == "HHbbZZ2L2Nu":
            file_path = "{}/RadionHHbbwww_2L2nu_narrow_M-{}.root".format(
                base_path, str(mass))
        ...
        # Create a 2D histogram for genpartFlav of leading and subleading
        # hadronic taus
        n_bins = 8
        hist2d = ROOT.TH2F("hist2d", "2D Normalized (%) {} ({} TeV);;".format(
            signal, str(mass / 1000)), n_bins, -1, 7, n_bins, -1, 7)

        # Loop over events and fill the histogram for the fully hadronic
        # channel (channel == 0)
        for event in tree:
            if (
                (
                    event.HTTvis_deltaR > 0) and (
                    event.HTTvis_deltaR < 1.5) and (
                    event.ngood_MediumJets == 0) and (
                    event.Hbb_Loose == 1) and (
                        event.X_m >= 750) and (
                            event.X_m <= 5010) and (
                                abs(
                                    event.Hbb_met_phi) > 1) and (
                                        event.HTTvis_m > 20)):
                total_events_SR += 1
                if (event.channel == 0):
            ...
            if (
                        ...
                        event.Hbb_Loose == 1) and (
                            event.X_m >= 750) and (
                                event.X_m <= 5010) and (
                                    abs(
                                        event.Hbb_met_phi) > 1) and (
                                            event.HTTvis_m > 20)):  # Select fully hadronic channel
                ...
                if (event.miniiso_addlepton_vetoflag_all == 0):
                    ...
                    # Fill the 2D histogram with the genpartFlav of leading and
                    # subleading taus
        ...
        print(("Fully Hadronic vetoed events in SR for {} is ".format(signal),
               total_events_SR_tt_rejected,
               "....percentage of total is... ",
               (float(total_events_SR_tt_rejected * 100) / total_events_SR_tt)))
        print(("Tau-Electron vetoed events in SR for {} is ".format(signal),
               total_events_SR_et_rejected,
               "....percentage of total is... ",
               (float(total_events_SR_et_rejected * 100) / total_events_SR_et)))
        print(("Tau-Muon vetoed events in SR for {} is ".format(signal),
               total_events_SR_mt_rejected,
               "....percentage of total is... ",
               (float(total_events_SR_mt_rejected * 100) / total_events_SR_mt)))

        # with open("lepveto-stdiso/output_percentages.txt", "w") as txtfile:
        # Write total events in SR
        txtfile.write(
            "\n\n\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n\n\nTotal events in SR for {} is {}\n".format(
                signal, total_events_SR))
        ...
        txtfile.write(
            "Fully Hadronic vetoed events in SR for {} is {} ....percentage of total is... {:.2f}%\n".format(
                signal,
                total_events_SR_tt_rejected,
                (float(
                    total_events_SR_tt_rejected *
                    ...
                    total_events_SR_tt)))
        txtfile.write(
            "Tau-Electron vetoed events in SR for {} is {} ....percentage of total is... {:.2f}%\n".format(
                signal,
                total_events_SR_et_rejected,
                (float(
                    total_events_SR_et_rejected *
                    ...
                    total_events_SR_et)))
        txtfile.write(
            "Tau-Muon vetoed events in SR for {} is {} ....percentage of total is... {:.2f}%\n".format(
                signal, total_events_SR_mt_rejected, (float(
                    total_events_SR_mt_rejected * 100) / total_events_SR_mt)))

        # Create a canvas to draw the histogram
        canvas = ROOT.TCanvas(
            "canvas", "2D Normalized Tau GenPartFlav - {} TeV {}".format(str(mass), signal), 800, 600)
        ROOT.gStyle.SetOptStat(0)

        canvas.SetLeftMargin(0.15)  # Increase the left margin
        canvas.SetRightMargin(0.1)  # Adjust the right margin if necessary
        # Adjust the bottom margin if x-axis labels need space
        canvas.SetBottomMargin(0.10)
        canvas.SetTopMargin(0.10)  # Adjust the top margin for the title

        # Set a single color palette (e.g., shades of blue)
        # You can switch to ROOT.kGrayScale for grayscale
        ROOT.gStyle.SetPalette(30)
        # Format to show the percentage with one decimal place
        ...
        labels = [
            "",
            "jets #rightarrow #tau_{h}",
            "e #rightarrow #tau_{h}",
            "#mu #rightarrow #tau_{h}",
            "#tau_{e} #rightarrow #tau_{h}",
            "#tau_{#mu} #rightarrow #tau_{h}",
            "#tau_{h} #rightarrow #tau_{h}",
            ""]
        for i in range(1, 8):
            # print(i)
        ...
        # Redraw with the text option to show percentages
        hist2d.Draw("COLZ TEXT")

        # Update and save the canvas with a filename reflecting the mass point
        # and signal
        output_filename = "lepveto-miniiso/tau_genpartFlav_2D_{}TeV_{}_lepveto.png".format(
            str(mass), signal)
        canvas.Update()
