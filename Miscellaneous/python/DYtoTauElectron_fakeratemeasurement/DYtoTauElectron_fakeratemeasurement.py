...
class TauToElectronFakeRate(Module):
    ...
    def endFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        print("\n\n\n>>>>>>>>>>>...END FILE........<<<<<<<<<<<<")
        print(("Numerator = ", self.numerator_count_file,
              " Denomiantor = ", self.denominator_count_file))
        if (self.denominator_count_file == 0):
            print("Denominator is zezo")
        else:
            print(("Fake rate = %.3f%%" % (
                float(self.numerator_count_file * 100) / self.denominator_count_file)))
        # print("Fake rate = ", float((self.numerator_count_file*100)/self.denominator_count_file))

    def endJob(self):
        print("\n\n\n>>>>>>>>>>>...Full DY Process........<<<<<<<<<<<<")
        print(("Numerator = ", self.numerator_count,
              " Denomiantor = ", self.denominator_count))
        print(("Fake rate (Electron)= %.3f%%" %
               (float(self.numerator_count * 100) / self.denominator_count)))
        # pass
    ...
    def analyze(self, event):
        def has_lepton_descendant(genparts, tau, target_pdgId):
            ...
            for gen in genparts:
                if gen.genPartIdxMother == tau._index:
                    ...
                    descendant = has_lepton_descendant(
                        genparts, gen, target_pdgId)
                    if descendant:
                        return descendant
            return None
        ...
        FatJet_enu = [x for x in FatJet if (x.pt_nom >= 200) and (
            abs(x.eta) < 2.5) and (x.jetId > 1) and (x.msoftdrop_nom >= 30)]
        if (len(FatJet_enu) == 0):
            return False
                    ...
                    abs(
                        x.dz) < 0.2) and (
                            x.idDecayModeNewDMs) and (
                                x.idDeepTau2018v2p5VSjet >= 4) and (
                                    x.idDeepTau2018v2p5VSe >= 2) and (
                                        x.idDeepTau2018v2p5VSmu >= 1)]  # The HPS tau id are not bitmps anymore
        boostedTau_enu = [x for x in boostedtaus if (x.pt > 20) and (
            abs(x.eta) < 2.5) and (x.rawDeepTau2018v2p7VSjet >= 0.85)]
        ...
        for tau in taus:
            ...
            electron_descendant = has_lepton_descendant(
                genparts, tau, 11)  # 11 for electron
            if electron_descendant:
                electron_from_tau = electron_descendant
            else:
                # Check for any lepton descendant to classify as hadronic or
                # leptonic
                if not has_lepton_descendant(genparts, tau, 13):  # 13 for muon
                    hadronic_tau = tau

        # Count denominator if we have one tau decaying to an electron and the
        # other hadronic
        if electron_from_tau and hadronic_tau:
            if (((electron_from_tau.pt > 10) and (abs(electron_from_tau.eta) < 2.5)) and (
                    (hadronic_tau.pt > 20) and (abs(hadronic_tau.eta) < 2.5))):
                # Apply event-level pre-selections both for numerator and
                # Denominator

                # print ("Filling Denominator")
                self.denominator_count += 1 * event.crossSectionWeighting * event.pileupWeighting
                self.denominator_count_file += 1 * \
                    event.crossSectionWeighting * event.pileupWeighting

                # Check if the electron from tau decay matches any of the
                # filtered taus or boosted taus
                matched = False
                for tau in Tau_enu:
                ...
                if matched:
                    ...
                    self.numerator_count += 1 * event.crossSectionWeighting * \
                        event.pileupWeighting  # Increment numerator count
...
if __name__ == "__main__":
    ...
    fnames = glob.glob(args.inputLocation + "/DYJetsToLL_M-50*.root")
    outputDir = args.outputLocation

    def mainModule(): return TauToElectronFakeRate(args.year)
    p = PostProcessor(
        args.tempStore,
        fnames,
        cut=None,
