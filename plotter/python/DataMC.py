...
def main():
	...
	parser.add_argument('--topRW',help='Scaling the top MC by a flat factor',action='store_true')
	parser.add_argument('--computeTopWt',help='Compute the top MC flat factor', action='store_true')
	parser.add_argument('--storeshape',help='Store the shape in root file', action='store_true')
	parser.add_argument('--controlregion','-c',help='If it is control region then we donot plot signals',action='store_true')
...
#		from variables_topCR import *
	...
	fnames = list(set(glob.glob(args.Path + "/*.root"))-set(glob.glob(args.Path + "/*Graviton*.root"))-set(glob.glob(args.Path + "/*RadionToHHTo2B2*.root")))
	print(fnames)
	DatasetNameList=[]
	DYlow_HistoList=[]
	DY_HistoList=[]
	...
	logScaleVarList = ["Fatjet_pnet_bbvsqcd","X_pt","X_m", "FatJet_pt_nom[index_gFatJets[0]]","METcorrected_pt"]	

	########################################################################################################
	#If we want to store the shapes then open and recreate a root file
	if (args.storeshape):
		shape_file = ROOT.TFile(args.Channel.upper()+"Plots/"+args.Sub+'/'+"shaperoot_"+args.Channel+".root", "RECREATE")
	...
	for variable in args.variables:
		...
		for index in range(len(DatasetNameList)):
			...
			if (search("MET", DatasetNameList[index]) or search("Run", DatasetNameList[index])):
				...
				#DatasetNameList[index],theWeight='1'+"*((FatJet_particleNetLegacy_mass[index_gFatJets[0]] > 150) || (FatJet_particleNetLegacy_mass[index_gFatJets[0]] < 100))")
		...
		for index in range(len(SignalNameList)):
			...
			SignalObjects[SignalNameList[index]].StandardDraw(SignalObjects[SignalNameList[index]].RootFileName,
				...
				#SignalNameList[index],theWeight=args.Weight+"*((FatJet_particleNetLegacy_mass[index_gFatJets[0]] > 150) || (FatJet_particleNetLegacy_mass[index_gFatJets[0]] < 100))")
        ...
        ###########################CLUB ALL HISTS##################################
		totalbkg_Histo = clubHistograms(DY_HistoList+DYlow_HistoList+ST_HistoList+TT_HistoList+QCD_HistoList+WJets_HistoList+DiBoson_HistoList,DatasetObjects)
		#######################DYlow-Histograms####################################
		...
		DY_Histo = clubHistograms(DY_HistoList+DYlow_HistoList,DatasetObjects)
...
#        print ("Number of events of Data = ",Data_Histo.Integral())
		...
		print ("Number of events in TTBar (incl over and under) = ",TT_Histo.Integral(0,TT_Histo.GetNbinsX()+1))
		print ("TTbar Component Split : >>>>>>>>>>>>>>>>>>")
		print ("Number of events in TTBarHadronic (incl over and under) = ",TTHad_Histo.Integral(0,TTHad_Histo.GetNbinsX()+1))
		print ("Number of events in TTBarSemileptonic (incl over and under) = ",TTSem_Histo.Integral(0,TTSem_Histo.GetNbinsX()+1))
		print ("Number of events in TTBar2L2Nu (incl over and under) = ",TT2L2Nu_Histo.Integral(0,TT2L2Nu_Histo.GetNbinsX()+1))
		print (">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
		print ("Number of events in WJets (incl over and under) = ",WJets_Histo.Integral(0,WJets_Histo.GetNbinsX()+1))
		print ("Number of events in DY + DY-low (incl over and under) = ",DY_Histo.Integral(0,DY_Histo.GetNbinsX()+1))
		#print ("Number of events in lowMassDY (incl over and under) = ",DYlow_Histo.Integral(0,DYlow_Histo.GetNbinsX()+1))
		print ("Number of events in ST (incl over and under) = ",ST_Histo.Integral(0,ST_Histo.GetNbinsX()+1))
		print ("Number of events in QCD (incl over and under) = ",QCD_Histo.Integral(0,QCD_Histo.GetNbinsX()+1))
		print ("Number of Diboson (incl over and under) = ",DiBoson_Histo.Integral(0,DiBoson_Histo.GetNbinsX()+1))
		print ("Total Background (incl over and under) = ",TT_Histo.Integral(0,TT_Histo.GetNbinsX()+1)+WJets_Histo.Integral(0,WJets_Histo.GetNbinsX()+1)+DY_Histo.Integral(0,DY_Histo.GetNbinsX()+1)+ST_Histo.Integral(0,ST_Histo.GetNbinsX()+1)+QCD_Histo.Integral(0,QCD_Histo.GetNbinsX()+1)+DiBoson_Histo.Integral(0,DiBoson_Histo.GetNbinsX()+1))
		...
		print ("Number of events of Data (incl over and under) = ",Data_Histo.Integral(0,Data_Histo.GetNbinsX()+1))

		print("\n\n\n >>>>>>>>>>>>>>>>>>>Error and Integral<<<<<<<<<<<<<<<<<<<<<<<<<")
		error_bkg = ROOT.Double(0)
		integral_bkg = totalbkg_Histo.IntegralAndError(0,totalbkg_Histo.GetNbinsX()+1, error_bkg, "")
		print("Background integral = {} +- {}".format(integral_bkg, error_bkg)) 
		error_data = ROOT.Double(0)
		integral_data = Data_Histo.IntegralAndError(0,Data_Histo.GetNbinsX()+1, error_data, "")
		print("Data integral = {} +- {}".format(integral_data, error_data))
		print(">>>>>>>>>>>>>>>>>>>Error and Integral<<<<<<<<<<<<<<<<<<<<<<<<<\n\n\n")  


		if (args.storeshape):
			shape_hist = clubHistograms(TT_HistoList+ST_HistoList+DY_HistoList+DYlow_HistoList+QCD_HistoList+WJets_HistoList+DiBoson_HistoList,DatasetObjects)
			shape_hist.Sumw2()
			shape_hist.Scale((1.0/(TT_Histo.Integral(0,TT_Histo.GetNbinsX()+1)+WJets_Histo.Integral(0,WJets_Histo.GetNbinsX()+1)+DY_Histo.Integral(0,DY_Histo.GetNbinsX()+1)+ST_Histo.Integral(0,ST_Histo.GetNbinsX()+1)+QCD_Histo.Integral(0,QCD_Histo.GetNbinsX()+1)+DiBoson_Histo.Integral(0,DiBoson_Histo.GetNbinsX()+1))))
			shape_hist.Sumw2()
		...
		if (args.computeTopWt):
			...
			print(">>>>>>>>>>>>>>>>>>>>>>>>>REFINED TOP REWEIGHTING FACTOR>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
			print("Data = ", Data_Integral)
			print("MC Total = ",MC_Integral)
			print("Total TT+ST = ",Top_Integral)
			Top_Weight = ((Data_Integral-MC_Integral+Top_Integral)/(Top_Integral))
			Numerator_fe = (Data_Integral-MC_Integral)
			ratio_fe = (Data_Integral-MC_Integral)/Top_Integral

			print("Correction factor for channel ",args.Channel," is = ",Top_Weight)
			print("dataErr = ",dataErr.value)
			print("MCErr = ",MCErr.value)
			print("TopErr = ",TopErr.value)
			Top_Weight_Error = math.sqrt(pow(((ratio_fe*dataErr.value)/(Numerator_fe)),2)+pow(((ratio_fe*MCErr.value)/(Numerator_fe)),2)+pow(((ratio_fe*TopErr.value)/(Top_Integral)),2))
...
#
		...
		ratioHist, ratioError, arrows  = MakeRatioHistograms(Data_Histo,backgroundStack,variable)
		ratioPad.cd()
...
#       
		#Delete
		maxi =max(backgroundStack.GetMaximum(),Data_Histo.GetMaximum(),Signal_Histo[0].GetMaximum(),Signal_Histo[1].GetMaximum(),Signal_Histo[2].GetMaximum(),Signal_Histo[3].GetMaximum())
	...
	##############################Legend############################    
		...
		if(args.controlregion):
			...
			theLegend.AddEntry(Signal_Histo[0],'1TeV('+str(signalXS)+'pb)','l')
			theLegend.AddEntry(Signal_Histo[1],'2TeV('+str(signalXS)+'pb)','l')
			theLegend.AddEntry(Signal_Histo[2],'3TeV('+str(signalXS)+'pb)','l')
			theLegend.AddEntry(Signal_Histo[3],'4TeV('+str(signalXS)+'pb)','l')
...
#			print ("Enter a valid channel")
		...
		if args.Channel in output_dirs:
		    ...
		    theCanvas.SaveAs(output_dir + variableFileNameDictionary[variable] + '_' + args.Channel + '_' + args.year + '.png')
		    theCanvas.SaveAs(output_dir + variableFileNameDictionary[variable] + '_' + args.Channel + '_' + args.year + '.pdf')
		else:
		    print("Enter a valid channel")
