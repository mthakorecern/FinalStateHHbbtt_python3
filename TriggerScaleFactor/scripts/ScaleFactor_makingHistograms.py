...
class dataEfficiencty(Module):
    ...
    def analyze(self, event):
        ...
        goodMuons = filter(lambda j : j.pt > 30,Muons)

        METcorrected_pt_phi = ROOT.METXYCorr_Met_MetPhi(event.MET_T1_pt, event.MET_T1_phi, event.run,"2016nonAPV",True,True, event.PV_npvs)
...
class mcEfficiencty(Module):
    ...
    def analyze(self, event):
        ...
        METcorrected_pt_phi = ROOT.METXYCorr_Met_MetPhi(event.MET_T1_pt, event.MET_T1_phi,event.run,"2016nonAPV",True,event.PV_npvs,True,False)
        METcorrected_pt = METcorrected_pt_phi[0]
        METcorrected_phi = METcorrected_pt_phi[1]
        ...
        if(event.HLT_PFMETNoMu110_PFMHTNoMu110_IDTight 
          ...
          #self.h_mc_numerator.Fill(METcorrected_pt)
          #self.h_mcpassmettrig.Fill(1)
...
fnames_mc = glob.glob(args.mcPath + "/*.root")
print (fnames_mc)
