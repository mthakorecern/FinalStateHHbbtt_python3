...
class mettrigsf(Module):
	...
	def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
		...
		self.out.branch("nele_0p1","I")
		self.out.branch("nele_0p4","I")
		self.out.branch("npho_0p1","I")
		self.out.branch("npho_0p4","I")
		self.out.branch("ntau_0p1","I")
		self.out.branch("ntau_0p4","I")
	...
	def analyze(self, event):
		...
		if (args.year == "2018"):
			if ((self.isData==True) and (event.run>=3190770)):
		...
		def MuonElectronPhotonOverlap0p1(Object_enu): # Function used by electron(muon) cleaning vs AK8 > 0.8
			self.lepFV.SetPtEtaPhiM(Object_enu[1].pt,Object_enu[1].eta,Object_enu[1].phi,0.0)
			deltaR = self.lepFV.DeltaR(Muons_enu[0][1].p4())
			if deltaR > 0.1:
				return True
			else:
				return False	

		def MuonElectronPhotonOverlap0p4(Object_enu): # Function used by electron(muon) cleaning vs AK8 > 0.8
			self.lepFV.SetPtEtaPhiM(Object_enu[1].pt,Object_enu[1].eta,Object_enu[1].phi,0.0)
			deltaR = self.lepFV.DeltaR(Muons_enu[0][1].p4())
			if deltaR > 0.4:
				return True
		...
		def MuonTauOverlap0p1(Object_enu): # Function used by electron(muon) cleaning vs AK8 > 0.8
			self.lepFV.SetPtEtaPhiM(Object_enu[1].pt,Object_enu[1].eta,Object_enu[1].phi,Object_enu[1].mass)
			deltaR = self.lepFV.DeltaR(Muons_enu[0][1].p4())
			if deltaR > 0.1:
				return True
			else:
		...
		def MuonTauOverlap0p4(Object_enu): # Function used by electron(muon) cleaning vs AK8 > 0.8
			self.lepFV.SetPtEtaPhiM(Object_enu[1].pt,Object_enu[1].eta,Object_enu[1].phi,Object_enu[1].mass)
			deltaR = self.lepFV.DeltaR(Muons_enu[0][1].p4())
			if deltaR > 0.4:
				return True
		...
		Electron = Collection(event,'Electron','nElectron')
		Electron_enu = filter(lambda x: x[1].pt > 10 and ((int(x[1].cutBased))>=2) and (abs(x[1].eta)<2.4), enumerate(Electron))
		Electron_enu_0p1 = filter(lambda x:MuonElectronPhotonOverlap0p1,Electron_enu)
		Electron_enu_0p4 = filter(lambda x:MuonElectronPhotonOverlap0p4,Electron_enu)

		Photon = Collection(event,'Photon','nPhoton')
		Photon_enu = filter(lambda x: x[1].pt > 20 and ((int(x[1].cutBased))>=1) and (abs(x[1].eta)<2.4), enumerate(Photon))
		Photon_enu_0p1 = filter(lambda x:MuonElectronPhotonOverlap0p1,Photon_enu)
		Photon_enu_0p4 = filter(lambda x:MuonElectronPhotonOverlap0p4,Photon_enu)
		...
		Tau_enu_0p1 = filter(lambda x:MuonTauOverlap0p1,Tau_enu)
		Tau_enu_0p4 = filter(lambda x:MuonTauOverlap0p4,Tau_enu)

		self.out.fillBranch("nele_0p1",len(Electron_enu_0p1))
		self.out.fillBranch("nele_0p4",len(Electron_enu_0p4))
		self.out.fillBranch("npho_0p1",len(Photon_enu_0p1))
		self.out.fillBranch("npho_0p4",len(Photon_enu_0p4))
		self.out.fillBranch("ntau_0p1",len(Tau_enu_0p1))
		self.out.fillBranch("ntau_0p4",len(Tau_enu_0p4))
		#if(refTrigger):
		#try:
		if (args.year=="2016"):
