#!/usr/bin/env python
# This script is used for files that are pre-processed to have a good
# Data-MC agreement in the SingleMuon Dataset
import argparse
import json
import numpy as np
import glob
...
class dataEfficiencty(Module):
    ...
    def beginJob(self, histFile=None, histDirName=None):
        ...
        self.h_data_denominator = ROOT.TH1F(
            "h_data_denominator",
            "; E_{T}^{miss} [GeV]",
            self.nbin,
            self.binEdges)
        self.h_data_numerator = ROOT.TH1F(
            "h_data_numerator",
            "; E_{T}^{miss} [GeV]",
            self.nbin,
            self.binEdges)
    ...
    def analyze(self, event):
        ...
        if ((FatJet[event.fatjet_index].msoftdrop < 30) or (
                FatJet[event.fatjet_index].pt_nom <= 200) or (event.ngood_MediumJets != 0)):
            return False
        ...
        if (args.year == "2016"):
            ...
            # if(metTrigger and ((goodMuons[0].p4().DeltaR(FatJet[0].p4())) >
            # 0.8)):
        ...
        # except:
            ...
            # if(metTriggerSansBeamHaloCleaned and
            # ((goodMuons[0].p4().DeltaR(FatJet[0].p4())) > 0.8)):
            if (event.met_trig_bool == 1):
                self.h_data_numerator.Fill(event.METcorrected_pt)
...
class mcEfficiencty(Module):
    ...
    def beginJob(self, histFile=None, histDirName=None):
        ...
        self.h_mc_denominator = ROOT.TH1F(
            "h_mc_denominator",
            "; E_{T}^{miss} [GeV]",
            self.nbin,
            self.binEdges)
        self.h_mc_numerator = ROOT.TH1F(
            "h_mc_numerator",
            "; E_{T}^{miss} [GeV]",
            self.nbin,
            self.binEdges)
        ...
        # Add a customization to different yrs
        self.xsJSON = os.environ['CMSSW_BASE'] + \
            "/src/MetaData/xsJSON/%s_Samples.json" % (self.year)
        with open(self.xsJSON, 'r') as xsjson:
            self.xsInfo = json.load(xsjson)

        # Add a customization to different yrs
        self.countsJSON = os.environ['CMSSW_BASE'] + \
            '/src/MetaData/SumOfGenWeights/%s_weight.json' % (self.year)
        with open(self.countsJSON, 'r') as countsjson:
            self.countsInfo = json.load(countsjson)

    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        print((" The inputFile name gotten from the beginFile fn : ",
              type(inputFile), "   ", inputFile.GetName()))
        filenameForJson = (
            inputFile.GetName().strip().split('/')[-1]).split('.')[-2]
        ...
        print((" Filename for JSON : ", self.filename, " XS = ",
              self.XS, " sumofgenwts = ", self.totalNumberOfEvents))
    ...
    def analyze(self, event):
        ...
        if ((FatJet[event.fatjet_index].msoftdrop < 30) or (
                FatJet[event.fatjet_index].pt_nom <= 200) or (event.ngood_MediumJets != 0)):
            # if ((FatJet[event.fatjet_index[0]].msoftdrop<30) or
            # (FatJet[event.fatjet_index[0]].pt_nom<=200)):
            return False

        self.h_mc_denominator.Fill(
            event.METcorrected_pt,
            event.FinalWeighting *
            event.pileupWeighting *
            event.combinedWZgenPtDeborahWeight *
            event.Top_pt_rwt)
...
parser = argparse.ArgumentParser(
    description='Script create efficiency histograms for data and mc to be used to compute scalefactors (currently wotj )')
...
def data(): return dataEfficiencty()
def mc(): return mcEfficiencty(args.year)
...
print([(nameStrip.strip().split('/')[-1]).split('.')[-2]
      for nameStrip in fnames_mc])
...
if (args.year == "2016"):
    ...
    post2016_MC = PostProcessor(
        ...
        modules=[
            mc()],
        noOut=True,
        histFileName="2016/2016MC_MCHistos.root",
        histDirName="MCHistos")
    ...
    post2016_RunF = PostProcessor(
        "2016/.",
        glob.glob(
            args.mcPath +
            "/SingleMu_Run2016F*.root"),
        cut=None,
        branchsel=None,
        modules=[
            data()],
        noOut=True,
        histFileName="2016/2016RunF_DataHistos.root",
        histDirName="DataHistos")
    ...
    post2016_RunG = PostProcessor(
        "2016/.",
        glob.glob(
            args.mcPath +
            "/SingleMu_Run2016G*.root"),
        cut=None,
        branchsel=None,
        modules=[
            data()],
        noOut=True,
        histFileName="2016/2016RunG_DataHistos.root",
        histDirName="DataHistos")
    post2016_RunG.run()
    ...
    post2016_RunH = PostProcessor(
        "2016/.",
        glob.glob(
            args.mcPath +
            "/SingleMu_Run2016H*.root"),
        cut=None,
        branchsel=None,
        modules=[
            data()],
        noOut=True,
        histFileName="2016/2016RunH_DataHistos.root",
        histDirName="DataHistos")
    post2016_RunH.run()
    ...
    APV2016_MC = PostProcessor(
        ...
        modules=[
            mc()],
        noOut=True,
        histFileName="2016APV/2016APVMC_MCHistos.root",
        histDirName="MCHistos")
    APV2016_MC.run()
    ...
    Run2016B_ver1_HIPM = PostProcessor(
        "2016APV/.",
        glob.glob(
            args.mcPath +
            "/SingleMu_Run2016B-ver1_HIPM*.root"),
        cut=None,
        branchsel=None,
        modules=[
            data()],
        noOut=True,
        histFileName="2016APV/Run2016B_ver1_HIPM_DataHistos.root",
        histDirName="DataHistos")
    ...
    Run2016B_ver2_HIPM = PostProcessor(
        "2016APV/.",
        glob.glob(
            args.mcPath +
            "/SingleMu_Run2016B-ver2_HIPM*.root"),
        cut=None,
        branchsel=None,
        modules=[
            data()],
        noOut=True,
        histFileName="2016APV/Run2016B_ver2_HIPM_DataHistos.root",
        histDirName="DataHistos")
    ...
    Run2016C_HIPM = PostProcessor(
        "2016APV/.",
        glob.glob(
            args.mcPath +
            "/SingleMu_Run2016C-HIPM*.root"),
        cut=None,
        branchsel=None,
        modules=[
            data()],
        noOut=True,
        histFileName="2016APV/Run2016C_HIPM_DataHistos.root",
        histDirName="DataHistos")
    Run2016C_HIPM.run()
    ...
    Run2016D_HIPM = PostProcessor(
        "2016APV/.",
        glob.glob(
            args.mcPath +
            "/SingleMu_Run2016D-HIPM*.root"),
        cut=None,
        branchsel=None,
        modules=[
            data()],
        noOut=True,
        histFileName="2016APV/Run2016D_HIPM_DataHistos.root",
        histDirName="DataHistos")
    Run2016D_HIPM.run()
    ...
    Run2016E_HIPM = PostProcessor(
        "2016APV/.",
        glob.glob(
            args.mcPath +
            "/SingleMu_Run2016E-HIPM*.root"),
        cut=None,
        branchsel=None,
        modules=[
            data()],
        noOut=True,
        histFileName="2016APV/Run2016E_HIPM_DataHistos.root",
        histDirName="DataHistos")
    Run2016E_HIPM.run()
    ...
    Run2016F_HIPM = PostProcessor(
        "2016APV/.",
        glob.glob(
            args.mcPath +
            "/SingleMu_Run2016F-HIPM*.root"),
        cut=None,
        branchsel=None,
        modules=[
            data()],
        noOut=True,
        histFileName="2016APV/Run2016F_HIPM_DataHistos.root",
        histDirName="DataHistos")
    Run2016F_HIPM.run()

elif (args.year == "2018"):
    print("Creating/Running post processor for 2018 MC")
    post2018_MC = PostProcessor(
        "2018/.",
        fnames_mc,
        cut=None,
        branchsel=None,
        modules=[
            mc()],
        noOut=True,
        histFileName="2018/2018MC_MCHistos.root",
        histDirName="MCHistos")
    post2018_MC.run()

    print("Creating/Running post processor for 2018 Data - SingleMu_Run2018A")
    Run2018A = PostProcessor(
        "2018/.",
        glob.glob(
            args.mcPath +
            "/SingleMu_Run2018A*.root"),
        cut=None,
        branchsel=None,
        modules=[
            data()],
        noOut=True,
        histFileName="2018/Run2018A_DataHistos.root",
        histDirName="DataHistos")
    Run2018A.run()

    print("Creating/Running post processor for 2018 Data - SingleMu_Run2018B")
    Run2018B = PostProcessor(
        "2018/.",
        glob.glob(
            args.mcPath +
            "/SingleMu_Run2018B*.root"),
        cut=None,
        branchsel=None,
        modules=[
            data()],
        noOut=True,
        histFileName="2018/Run2018B_DataHistos.root",
        histDirName="DataHistos")
    Run2018B.run()

    print("Creating/Running post processor for 2018 Data - SingleMu_Run2018C")
    Run2018C = PostProcessor(
        "2018/.",
        glob.glob(
            args.mcPath +
            "/SingleMu_Run2018C*.root"),
        cut=None,
        branchsel=None,
        modules=[
            data()],
        noOut=True,
        histFileName="2018/Run2018C_DataHistos.root",
        histDirName="DataHistos")
    Run2018C.run()
    ...
    Run2018D = PostProcessor(
        "2018/.",
        glob.glob(
            args.mcPath +
            "/SingleMu_Run2018D*.root"),
        cut=None,
        branchsel=None,
        modules=[
            data()],
        noOut=True,
        histFileName="2018/Run2018D_DataHistos.root",
        histDirName="DataHistos")
    Run2018D.run()
    ...
    post2017_MC = PostProcessor(
        ...
        modules=[
            mc()],
        noOut=True,
        histFileName="2017/2017MC_MCHistos.root",
        histDirName="MCHistos")
    post2017_MC.run()

    print("Creating/Running post processor for 2017 Data - SingleMu_Run2017B")
    Run2017B = PostProcessor(
        "2017/.",
        glob.glob(
            args.mcPath +
            "/SingleMu_Run2017B*.root"),
        cut=None,
        branchsel=None,
        modules=[
            data()],
        noOut=True,
        histFileName="2017/Run2017B_DataHistos.root",
        histDirName="DataHistos")
    Run2017B.run()

    print("Creating/Running post processor for 2017 Data - SingleMu_Run2017C")
    Run2017C = PostProcessor(
        "2017/.",
        glob.glob(
            args.mcPath +
            "/SingleMu_Run2017C*.root"),
        cut=None,
        branchsel=None,
        modules=[
            data()],
        noOut=True,
        histFileName="2017/Run2017C_DataHistos.root",
        histDirName="DataHistos")
    Run2017C.run()

    print("Creating/Running post processor for 2017 Data - SingleMu_Run2017D")
    Run2017D = PostProcessor(
        "2017/.",
        glob.glob(
            args.mcPath +
            "/SingleMu_Run2017D*.root"),
        cut=None,
        branchsel=None,
        modules=[
            data()],
        noOut=True,
        histFileName="2017/Run2017D_DataHistos.root",
        histDirName="DataHistos")
    Run2017D.run()

    print("Creating/Running post processor for 2017 Data - SingleMu_Run2017E")
    Run2017E = PostProcessor(
        "2017/.",
        glob.glob(
            args.mcPath +
            "/SingleMu_Run2017E*.root"),
        cut=None,
        branchsel=None,
        modules=[
            data()],
        noOut=True,
        histFileName="2017/Run2017E_DataHistos.root",
        histDirName="DataHistos")
    Run2017E.run()

    print("Creating/Running post processor for 2017 Data - SingleMu_Run2017F")
    Run2017F = PostProcessor(
        "2017/.",
        glob.glob(
            args.mcPath +
            "/SingleMu_Run2017F*.root"),
        cut=None,
        branchsel=None,
        modules=[
            data()],
        noOut=True,
        histFileName="2017/Run2017F_DataHistos.root",
        histDirName="DataHistos")
    Run2017F.run()
