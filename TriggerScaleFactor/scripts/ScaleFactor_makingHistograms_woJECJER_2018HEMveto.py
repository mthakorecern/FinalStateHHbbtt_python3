	...
	def analyze(self, event):
 		def applyPOGselectionToAK4_for2018Veto(ak4Object_enu):
			if (((ak4Object_enu[1].pt>15) and (ak4Object_enu[1].pt<50)) and (ak4Object_enu[1].eta < -1.3) and (ak4Object_enu[1].eta > -3.2) and (ak4Object_enu[1].phi < -0.87) and (ak4Object_enu[1].phi > -1.57)):
				#if ((ak4Object_enu[1].jetId>=6) and (ak4Object_enu[1].puId>=4)):
				#Loose ID (not Looselepveto)
		...
		if (args.year == "2018"):
			#print ("VETO procedure for Data")
						
			if (event.run>=319077):
				Jet = Collection(event,'Jet','nJet')
				#print("This is beyond or equal to 319077 = ", event.run)
				Jet_Vetoenu = filter(applyPOGselectionToAK4_for2018Veto,enumerate(Jet))
				#FatJet_Vetoenu = filter(lambda x: (x[1].pt_nom > 200) and (x[1].eta < -1.3) and (x[1].eta > -3.2) and (x[1].phi < -0.87) and (x[1].phi > -1.57) and (x[1].jetId>=6) and (x[1].msoftdrop_nom>=30), enumerate(FatJet))
				#FatJet_Vetoenu = filter(lambda x: (x[1].pt_nom > 200) and (x[1].eta < -1.1) and (x[1].eta > -2.7) and (x[1].phi < -0.67) and (x[1].phi > -1.77) and (x[1].jetId>=6) and (x[1].msoftdrop_nom>=30), enumerate(FatJet))
				#use tight veto (instead of tight lep veto)
				FatJet_Vetoenu = filter(lambda x: (x[1].pt > 200) and (x[1].eta < -1.1) and (x[1].eta > -2.7) and (x[1].phi < -0.67) and (x[1].phi > -1.77) and (x[1].jetId>1) and (x[1].msoftdrop>=30), enumerate(FatJet))				
...
class mcEfficiencty(Module):
	...
	def analyze(self, event):
		...
		FatJet = Collection(event,'FatJet','nFatJet')
		if (args.year == "2018"):
			#print ("VETO procedure for MC")
			#self.counter = self.counter+1
			#frac = (float(self.counter)/float(self.totalEvents))
			#Lets implement random number event selection
