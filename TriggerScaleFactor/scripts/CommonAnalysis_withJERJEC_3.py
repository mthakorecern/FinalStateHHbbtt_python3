...
class mettrigsf(Module):
    ...
    def analyze(self, event):
        ...
        def applyPOGselectionToAK4_for2018Veto(ak4Object_enu):
            if (
                (
                    (ak4Object_enu[1].pt_nom > 15) and (
                        ak4Object_enu[1].pt_nom < 50)) and (
                    ak4Object_enu[1].eta < -
                    1.3) and (
                    ak4Object_enu[1].eta > -
                    3.2) and (
                ...
                if ((ak4Object_enu[1].jetId > 1) and (
                    ...
                    return True
            if ((ak4Object_enu[1].pt_nom >= 50) and (ak4Object_enu[1].eta < -
        ...
        if (args.year == "2018"):
            ...
            if ((self.isData) and (event.run >= 319077)):
                ...
                Jet_Vetoenu = list(
                    filter(
                        applyPOGselectionToAK4_for2018Veto,
                        enumerate(Jet)))
                FatJet_Vetoenu = [
        ...
        # FatJet and Jet overlap, separation > 1.2 (0.8 + 0.4)
        def FatJetMuonOverlap(muonObject_enu):
            self.muonFV.SetPtEtaPhiM(
                muonObject_enu[1].pt,
                muonObject_enu[1].eta,
                muonObject_enu[1].phi,
                muonObject_enu[1].mass)
        ...
        Muons_enu = [j for j in enumerate(Muons) if ((j[1].pt > 30) and (
            j[1].pfRelIso04_all < 0.15) and (j[1].tightId) and (abs(j[1].eta) < 2.4))]
        FatJet_enu = [x for x in enumerate(FatJet) if (
            (x[1].pt > 170) and (abs(x[1].eta) < 2.5) and (x[1].jetId > 1))]
        if (len(FatJet_enu) == 0):
            return False
        ...
        Muons_enu = list(filter(FatJetMuonOverlap, Muons_enu))
        ...
        if search("TTTo", self.filename):
            ...
            # Filter the genPart collection to pick out top (pdgid is 6) and
            # make sure top quark in MC is defined at parton level, after
            # radiation and before decay, using the 'isLastCopy' i.e status 62
            top = list([gen for gen in genParticles if (
                gen.pdgId == 6) and (gen.status == 62)])
            if len(top) > 0:
                SF_top = 0.103 * \
                    np.exp(-0.0118 * top[0].pt) - 0.000134 * top[0].pt + 0.973

            # Repeat the same for antitop
            antitop = list([gen for gen in genParticles if (
                gen.pdgId == -6) and (gen.status == 62)])
            if len(antitop) > 0:
                SF_antitop = 0.103 * \
                    np.exp(-0.0118 * antitop[0].pt) - 0.000134 * antitop[0].pt + 0.973
        ...
        # except:
            ...
            # We are ONLY keeping METNoMu triggers
            metTrigger2017 = (
                event.HLT_PFMETNoMu120_PFMHTNoMu120_IDTight or event.HLT_MonoCentralPFJet80_PFMETNoMu120_PFMHTNoMu120_IDTight)
            if (metTrigger2017):
                self.out.fillBranch("met_trig_bool", 1)
            else:
                self.out.fillBranch("met_trig_bool", 0)
...
def call_postpoc(files):
    ...
    if (search("Run", filename)):
        ...
        def mainModule(): return mettrigsf(filename, args.year, True)
        def wandzWtModule(): return wandzgenptweight(filename, args.year, True)
    else:
        print(("This is a ", args.year, " MC file = ", filename))
        def mainModule(): return mettrigsf(filename, args.year, False)
        def wandzWtModule(): return wandzgenptweight(filename, args.year, False)
    ...
    if (args.year == "2018"):
        if (search("Run", filename)):
            ...
            p = PostProcessor(
                args.tempStore,
                [files],
                cut="(HLT_IsoMu24) && (METcorrected_pt>=150)",
                branchsel="input_drop.txt",
                modules=[
                    mainModule(),
                    wandzWtModule()],
                ...
                postfix="",
                noOut=False,
                outputbranchsel="output_drop.txt")
        else:
            p = PostProcessor(
                args.tempStore,
                [files],
    ...
    if (args.year == "2016APV"):
        if (search("Run", filename)):
            ...
            p = PostProcessor(
                args.tempStore,
                [files],
                cut="((HLT_IsoMu24) || (HLT_IsoTkMu24)) && (METcorrected_pt>=150)",
                branchsel="input_drop.txt",
                modules=[
                    mainModule(),
                    wandzWtModule()],
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Output of this script will be used for computing MET SFs. These outfiles are stored under (/hdfs/store/user/gparida/HHbbtt/Hadded_Skimmed_Files/Full_Production_CMSSW_13_0_13_Nov24_23/JetCorrBranchesAndMETSkimTrim_METSFs)')
    parser.add_argument(
        '--inputLocation',
        '-i',
    ...
    fnames = list(set(glob.glob(args.inputLocation +
                                "/*.root")) -
                  set(glob.glob(args.inputLocation +
                                "/*Radion*.root")) -
                  set(glob.glob(args.inputLocation +
                                "/*Graviton*.root")))
